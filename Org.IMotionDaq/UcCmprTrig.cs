...
namespace Org.IMotionDaq
{
    ...
    /// 位置比较触发器调试面板
    /// </summary>
    public partial class UcCmprTrig : UcRealTimeUI
    {
        public UcCmprTrig()
        {
            InitializeComponent();
        ...
        int maxTips = 100;
        IPlatModule_CmprTrg _module = null;
        ...
        void ShowTips(string txt)
        {
            ...
            if (InvokeRequired)
            {
                BeginInvoke(new dgShowTip(ShowTips), new object[] { txt });
                return;
            }
        ...
        public void SetCmprTigger(IPlatModule_CmprTrg module, string[] encChnIDs, string[] trgChnIDs)
        {
            _module = module;
        ...
        void AdjustUI()
        {
            ...
            int err = 0;
            int locX = 3, locY = 30;
            for (int i = 0; i < _module.CompareCount; i++)
            {
                ...
                ucChn.SetModuleChn(_module, i);
                locY += ucChn.Height + 3;
                gbChns.Controls.Add(ucChn);
            }
            locY = 30;
            for (int i = 0; i < _module.TriggerCount; i++)
            {
                ///通道ID标签
