...
{
    public partial class UcMotion : UcRealTimeUI
    {
        public UcMotion()
        {
            InitializeComponent();
        ...
        {
            _isFormLoaded = true;
            ucLineParam.SetParamDesribe(CosParams.Create("运动参数", typeof(MotionParam), cValueLimit.Non, null, false));
            ucArcParam.SetParamDesribe(CosParams.Create("运动参数", typeof(MotionParam), cValueLimit.Non, null, false));
            AdjustUI();
        }

        bool _isFormLoaded = false;
        IPlatModule_Motion _module = null;
        ...
        public void SetModuleInfo(IPlatModule_Motion module, string[] axisIDs)
        {
            _module = module;
            _axisIDs = axisIDs;
            if (_isFormLoaded)
                ...
                if (dgvLinePos.Rows.Count > 0) //更新直线查补控件
                    for (int i = 0; i < dgvLinePos.Rows.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(axisIDTxt))
                        {
                            ...
                            {
                                double currPos = 0;
                                int err = _module.GetEncPos(axisID, out currPos);
                                if (err != 0)
        ...
        bool _isMotionParamValid(MotionParam mp)
        {
            if (mp.acc <= 0)
            {
        ...
        private void btStartLiner_Click(object sender, EventArgs e)
        {
            ...
            MotionParam mp = (MotionParam)omp;
