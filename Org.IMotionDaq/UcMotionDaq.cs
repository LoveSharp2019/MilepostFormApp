...
{
    public partial class UcMotionDaq :UcRealTimeUI
    {
        public UcMotionDaq()
        {
            InitializeComponent();
        ...
        public void SetDevice(IPlatDevice_MotionDaq dev, string id)
        {
            _dev = dev;
            _id = id;
            if (_isFormLoaded)
        ...
        IPlatDevice_MotionDaq _dev = null;
        FormDios fmDios;
        ...
        void AdjustUI()
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action(AdjustUI));
                return;
            }
            ...
            else //更新设备信息
            {
                ...
                {
                    UcParamEdit ucParam = new UcParamEdit();
                ...
                if (_dev.DioMCount > 0)
                {
                    for (int i = 0; i < _dev.DioMCount; i++)
                ...
                if (_dev.CompareTriggerMCount > 0)
                {
                    for (int i = 0; i < _dev.CompareTriggerMCount; i++)
                ...
                if (_dev.AioMCount > 0)
                {
                    for (int i = 0; i < _dev.AioMCount; i++)
        ...
        void UpdateDevice()
        {
            ...
            lbDioCount.Text = _dev.DioMCount.ToString();
            lbMotionCount.Text = _dev.McMCount.ToString();
            lbAioCount.Text = _dev.AioMCount.ToString();
            lbCmprTrigCount.Text = _dev.CompareTriggerMCount.ToString();
            return;
