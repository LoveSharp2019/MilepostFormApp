...
namespace Org.IMotionDaq
{
    public partial class UcAIOPanel : UserControl
    {
        ...
        [Category("属性"), Description("模拟量输出面板"), Browsable(true)]
        ...
        public void AddIO(IPlatModule_AIO aio, int index, string name = null)
        {
            if (name == null)
        ...
        public void RemoveIO(IPlatModule_AIO aio, int index)
        {
            foreach (UcAIOChn uc in lstUcAio)
                if (uc.AIOModule == aio && index == uc.IOIndex)
                {
