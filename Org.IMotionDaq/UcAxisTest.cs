...
namespace Org.IMotionDaq
{
    public partial class UcAxisTest : UserControl
    {
        ...
        public void ShowTip(string txt)
        {
            ...
            if (InvokeRequired)
            {
                BeginInvoke(new dgShowTip(ShowTip), new object[] { txt });
                return;
            }
        ...
        public void SetAxis(IPlatModule_Motion module, int axis)
        {
            motion = module;
            this.axis = axis;
            if (isLoaded)
        ...
        public void AdjustAxisView()
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action(AdjustAxisView));
                return;
            }
            ////界面显示等级
            switch (DisplayMode)
        ...
        public void UpdateAllAxisData2UI() //更新轴状态到界面
        {
            if (InvokeRequired)
            {
                BeginInvoke(new Action(UpdateAxisUI));
                return;
            }
        ...
        bool ReadAxisParam(out string errInfo)
        {
            ...
            double posp = 0, posn = 0;
            err = motion.GetSoftLimit(axis, out posp, out posn);
            if (0 != err)
            {
                errInfo += "获取电机负限位参数失败，Error:" + motion.GetErrorInfo(err) + "\n";
                chkEnableS.Checked = false;
                numSnl.Enabled = false;//负限位数值
                numSpl.Enabled = false; //正限位数值
            }
            else
            {
                chkEnableS.Checked =
                numSnl.Enabled =
                numSpl.Enabled = true; //正限位数值

                numSnl.Value = Convert.ToDecimal(posn);
                numSpl.Value = Convert.ToDecimal(posp);
            }
        ...
        bool isLoaded = false;
        IPlatModule_Motion motion = null;
        int axis = -1;
        ...
        private void btSetCmd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                int err = motion.SetPrfPos(axis, cmdPos);
                if (err != 0)
        ...
        private void btSetFbk_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                int err = motion.SetEncPos(axis, fbkPos);
                if (err != 0)
        ...
        private void btWriteAxisMoveParam_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                MotionParam mp = new MotionParam();
        ...
        void RepeatLoop()
        {
            ...
            try
            {
                err = motion.GetEncPos(axis, out fbkPos);
                if (0 != err)
                {
        ...
        exit_loop:
            IsRepeating = false;
            BeginInvoke(new Action(RepeatExit));
            ShowTip("往复线程已停止");
        }
        ...
        private void btWriteAxisParam_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                errCode = motion.SetSoftLimit(axis, Convert.ToDouble(numSpl.Value), Convert.ToDouble(numSnl.Value));
                if (errCode != 0)
                {
                    isOK = false;
                    errInfo += "写入软限位参数失败,Error:" + motion.GetErrorInfo(errCode);
                    bc = Color.Red;
                }

                chkEnableS.BackColor = bc;
                numSpl.BackColor = bc;
                numSnl.BackColor = bc;
        ...
        {
            numSpl.Enabled = chkEnableS.Checked;
        }
