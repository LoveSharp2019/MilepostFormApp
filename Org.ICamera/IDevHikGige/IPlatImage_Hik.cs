...
namespace Org.ICamera.IDevHikGige
{
    public class IPlatImage_Hik : IPlat_Image
    {
        /// <summary>
        /// 
        /// </summary>
        ...
        internal IPlatImage_Hik(byte[] dataBytes, MyCamera.MV_FRAME_OUT_INFO_EX frameInfo, MyCamera cmr)
        {
        ...
        public IPlatImgPixFormat PixerFormat
        {
            get
            {
                switch (_frameInfo.enPixelType)
                {
                    case MyCamera.MvGvspPixelType.PixelType_Gvsp_Mono8:
                        return IPlatImgPixFormat.Mono8;
                    case MyCamera.MvGvspPixelType.PixelType_Gvsp_RGB8_Packed:
                        return IPlatImgPixFormat.RGB24;
                    case MyCamera.MvGvspPixelType.PixelType_Gvsp_RGB8_Planar:
                        return IPlatImgPixFormat.RGB24_P;
                    default:
                        return IPlatImgPixFormat.Unknown;
                }
        ...
        public int Save(string filePath, IPlatImgSuffixType fileType = IPlatImgSuffixType.Bmp)
        {
            ...
            try
            {
                if (fileType == IPlatImgSuffixType.Bmp || fileType == IPlatImgSuffixType.Jpg) //调用Hik提供的功能
                {
                    ...
                    MyCamera.MV_SAVE_IMAGE_PARAM_EX3 stSaveParam = new MyCamera.MV_SAVE_IMAGE_PARAM_EX3();
                    ...
                    if (fileType == IPlatImgSuffixType.Jpg) //if(stSaveParam.enImageType == MyCamera.MV_SAVE_IAMGE_TYPE.MV_Image_Jpeg)
                        stSaveParam.nJpgQuality = 99;

                    int err = _cmr.MV_CC_SaveImageEx3_NET(ref stSaveParam);
                    if (MyCamera.MV_OK != err)
                    {
                        return (int)ErrorDef.InvokeFailed;
                    }
                ...
                else if (fileType == IPlatImgSuffixType.Tif)
                {
                ...
                else if (fileType == IPlatImgSuffixType.Png)
                {
                    string fn = Path.GetFileNameWithoutExtension(filePath);
                    if (string.IsNullOrEmpty(fn))
                        return (int)ErrorDef.FolderUnExist;
