...
namespace Sys.UI
{
    public partial class Form_DevCellNameMgr : windowBase
    {

        public Form_DevCellNameMgr()
        {
            InitializeComponent();

            ucDioNames = new UcNamesEdit_Dio();
            ucDioNames.ContextMenuStrip = contextMenuStripEditName;
            ucAioNames = new UcNamesEdit_Aio();
            ucAioNames.ContextMenuStrip = contextMenuStripEditName;
            ucAxisNames = new UcNamesEdit_Motion();
        ...
        UcNamesEdit_Dio ucDioNames; //用于编辑DIO名称的控件
        UcNamesEdit_Aio ucAioNames;
        UcNamesEdit_Motion ucAxisNames;
        ...
        void _LoadCfg()
        {
            AppHubCenter.Instance.MDCellNameMgr.Load();
            _UpdateCfg2UI();
        }
        ...
        void _UpdateCfg2UI()
        {
            ...
            if (null != trigDevIDs)
                ...
                {
                    IPlatInitializable dev = AppHubCenter.Instance.InitorManager.GetInitor(trigDevID);
                    if (null == dev)
                        continue;

                    //if (!typeof(IJFDevice_LightController).IsAssignableFrom(dev.GetType())) //去除光源控制器设备
        ...
        void _SaveCfg()
        {
            AppHubCenter.Instance.MDCellNameMgr.Save();
        }
        ...
        {
            Form_DevModuleInfo fm = new Form_DevModuleInfo();
            fm.Text = "添加MotionDaq设备命名表";
            fm.SettingMode = DevModuleSettingMode.Add;
            if (DialogResult.OK != fm.ShowDialog())
                return;
            AppHubCenter.Instance.MDCellNameMgr.AddMotionDaqDevice(fm.DevID);
            AppHubCenter.Instance.MDCellNameMgr.SetAxisModuleCount(fm.DevID, fm.MotionCount);
            AppHubCenter.Instance.MDCellNameMgr.SetDioModuleCount(fm.DevID, fm.DioCount);
            AppHubCenter.Instance.MDCellNameMgr.SetAioModuleCount(fm.DevID, fm.AioCount);
            AppHubCenter.Instance.MDCellNameMgr.SetCmpTrigModuleCount(fm.DevID, fm.TrigCount);
        ...
        private void ToolStripMenuItemResetDevsModuleCount_Click(object sender, EventArgs e)
        {
            ...
            else if (nodeInfo.Categoty == DevNodeCategory.TrigCtrlDev)//更改触发控制器通道数量
            {
                ...
                //fm.Text = "修改触发器通道数量";
                //string[] devIDs = AppHubCenter.Instance.InitorManager.GetIDs(typeof(IJFDevice_TrigController)); //所有触发控制设备ID，过滤掉其中的光源控制器
                //List<string> trigDevIDs = new List<string>();
                //fm.SetDeviceID(nodeInfo.DevID);
                //fm.SettingMode = DevModuleSettingMode.Set;
                //fm.SetChannelTypes(new string[] { "触发通道数量:" });
                //int nCurrChns = AppHubCenter.Instance.MDCellNameMgr.GetTrigCtrlChannelCount(nodeInfo.DevID);
                //fm.ChannelCount = new int[] { nCurrChns };
                //if (DialogResult.OK != fm.ShowDialog())
                //    return;
                //AppHubCenter.Instance.MDCellNameMgr.SetTrigCtrlChannelCount(fm.DeviceID, fm.ChannelCount[0]);
            ...
            else if (nodeInfo.Categoty == DevNodeCategory.LightCtrlTDev) //更改光源控制器通道数量
            {
                ...
                //int lightChnCount = AppHubCenter.Instance.MDCellNameMgr.GetLightCtrlChannelCount(nodeInfo.DevID);
                //int trigChnCount = AppHubCenter.Instance.MDCellNameMgr.GetTrigCtrlChannelCount(nodeInfo.DevID);
                //fm.ChannelCount = new int[] { lightChnCount, trigChnCount };

                //if (DialogResult.OK != fm.ShowDialog())
                //    return;
                //AppHubCenter.Instance.MDCellNameMgr.SetLightCtrlChannelCount(fm.DeviceID, fm.ChannelCount[0]);
                //AppHubCenter.Instance.MDCellNameMgr.SetTrigCtrlChannelCount(fm.DeviceID, fm.ChannelCount[1]);
                //_UpdateModuleNode(currNode);
            }
        }
        ...
        private void ToolStripMenuItemDelectDev_Click(object sender, EventArgs e)
        {
            ...
            if (DialogResult.OK == MessageBox.Show("是否从命名表配置中删除设备:" + nodeInfo.DevID, "警告", MessageBoxButtons.OKCancel))
            {
                if (nodeInfo.Categoty == DevNodeCategory.MotionDaqDev)
                    AppHubCenter.Instance.MDCellNameMgr.RemoveMotionDaqDevice(nodeInfo.DevID);
                else if (nodeInfo.Categoty == DevNodeCategory.LightCtrlTDev)
                    AppHubCenter.Instance.MDCellNameMgr.RemoveLightCtrlDev(nodeInfo.DevID);
                else if (nodeInfo.Categoty == DevNodeCategory.TrigCtrlDev)
                    AppHubCenter.Instance.MDCellNameMgr.RemoveTrigCtrlDev(nodeInfo.DevID);
                //_UpdateCfg2UI();
                currNode.Remove();
        ...
        private void ToolStripMenuItemOpenCloseDev_Click(object sender, EventArgs e)
        {
            ...
            IPlatDevice dev = AppHubCenter.Instance.InitorManager.GetInitor(nodeInfo.DevID) as IPlatDevice;
            if (dev.IsDeviceOpen)
            {
        ...
        private void ToolStripMenuItemResetChannelCount_Click(object sender, EventArgs e)
        {
            ...
            DevNodeInfo nodeInfo = currNode.Tag as DevNodeInfo;
            AppDevCellNameManeger mgr = AppHubCenter.Instance.MDCellNameMgr;
            Form_SetModuleChnCount fm = new Form_SetModuleChnCount();
            if (nodeInfo.Categoty == DevNodeCategory.MotionModule)
            {
                fm.Category = ModuleCategory.Motion;
            ...
            {
                fm.Category = ModuleCategory.Trig;
            ...
            {
                fm.Category = ModuleCategory.Dio;
            ...
            {
                fm.Category = ModuleCategory.Aio;
        ...
        private void tvDevs_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)//判断右键
            {
                ...
                if (nodeInfo.Categoty == DevNodeCategory.MotionDaqDev)
                {
                    ...
                    if (AppHubCenter.Instance.InitorManager.GetIDs(typeof(IPlatDevice_MotionDaq)).Contains(nodeInfo.DevID))
                    {
                        contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Enabled = true;
                        IPlatDevice_MotionDaq dev = AppHubCenter.Instance.InitorManager.GetInitor(nodeInfo.DevID) as IPlatDevice_MotionDaq;
                        if (dev.IsDeviceOpen)
                ...
                else if (nodeInfo.Categoty == DevNodeCategory.LightCtrlTDev || nodeInfo.Categoty == DevNodeCategory.TrigCtrlDev)
                {
                    ...
                    //if (AppHubCenter.Instance.InitorManager.GetIDs(typeof(IJFDevice_TrigController)).Contains(nodeInfo.DevID))
                    //{
                    //    contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Enabled = true;
                    //    IJFDevice_TrigController dev = AppHubCenter.Instance.InitorManager.GetInitor(nodeInfo.DevID) as IJFDevice_TrigController;
                    //    if (dev.IsDeviceOpen)
                    //        contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Text = "关闭设备";
                    //    else
                    //        contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Text = "打开设备";
                ...
                {
                    //contextMenuDev.Items["ToolStripMenuItemResetDevsModuleCount"].Text = "修改通道数量";
                    //if (AppHubCenter.Instance.InitorManager.GetIDs(typeof(IJFDevice_LightControllerWithTrig)).Contains(nodeInfo.DevID))
                    //{
                    //    contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Enabled = true;
                    //    IJFDevice_LightControllerWithTrig dev = AppHubCenter.Instance.InitorManager.GetInitor(nodeInfo.DevID) as IJFDevice_LightControllerWithTrig;
                    //    if (dev.IsDeviceOpen)
                    //        contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Text = "关闭设备";
                    //    else
                    //        contextMenuDev.Items["ToolStripMenuItemOpenCloseDev"].Text = "打开设备";
            ...
            else if (e.Button == MouseButtons.Left) //左键
            {
                ...
                //MessageBox.Show("控件句柄个数：" + process.HandleCount.ToString());
            }
        }
        ...
        private void ToolStripMenuItemEditSave_Click(object sender, EventArgs e)
        {
            ...
            else //尝试保存并结束编辑状态
            {
                ...
                else if (nodeInfo.Categoty == DevNodeCategory.LightCtrlTDev)
                {
                    ...
                    //if (null != trigChannelNames && trigChannelNames.Length != 0)
                    //{
        ...
        private void toolStripMenuItemAddTrigCtrl_Click(object sender, EventArgs e)
        {
            ...
            //List<string> trigDevIDs = new List<string>();
            //foreach (string devID in devIDs)
            //    if (!typeof(IJFDevice_LightController).IsAssignableFrom(AppHubCenter.Instance.InitorManager.GetInitor(devID).GetType()))
            ...
            //AppHubCenter.Instance.MDCellNameMgr.AddTrigCtrlDev(fm.DeviceID);
            //AppHubCenter.Instance.MDCellNameMgr.SetTrigCtrlChannelCount(fm.DeviceID, fm.ChannelCount[0]);
        ...
        private void toolStripMenuItemAddLightTrig_Click(object sender, EventArgs e)
        {
            ...
            //FormAddMChnDev fm = new FormAddMChnDev();
            //fm.Text = "添加光源控制器设备_T";
            //string[] devIDs = AppHubCenter.Instance.InitorManager.GetIDs(typeof(IJFDevice_LightControllerWithTrig)); //所有触发控制设备ID，过滤掉其中的光源控制器
            //if (null == devIDs || 0 == devIDs.Length)
            //{
            ...
            //AppHubCenter.Instance.MDCellNameMgr.AddLightCtrlDev(fm.DeviceID);
            //AppHubCenter.Instance.MDCellNameMgr.SetLightCtrlChannelCount(fm.DeviceID, fm.ChannelCount[0]);
        ...
        private void ToolStripMenuItemAddLight_Click(object sender, EventArgs e)
        {
            MessageBox.Show("功能未开放");
            //FormAddMChnDev fm = new FormAddMChnDev();
            //fm.Text = "添加光源控制器设备";
            //string[] devIDs = AppHubCenter.Instance.InitorManager.GetIDs(typeof(IJFDevice_LightController)); //所有触发控制设备ID，过滤掉其中的光源控制器
            //if (null == devIDs || 0 == devIDs.Length)
            //{
            //    MessageBox.Show("设备列表中不存在光源控制器设备");
            //    return;
            //}
            //fm.SetOptionalDeviceIDs(devIDs);
            //fm.SetChannelTypes(new string[] { "开关通道数量:" });
            //if (DialogResult.OK != fm.ShowDialog())
            //    return;
