...
namespace Sys.UI
{
    ...
    public partial class Form_DevModuleInfo : Form
    {
        public Form_DevModuleInfo()
        {
            InitializeComponent();
        }
        ...
        {
            string[] devIDs = AppHubCenter.Instance.InitorManager.GetIDs(typeof(IPlatDevice_MotionDaq));//设备表中存在的设备ID（不是命名表）
            string[] allNamedDevIDs = AppHubCenter.Instance.MDCellNameMgr.AllMotionDaqDevices();//已在命名表中存在的DevID
        ...
        private void btOK_Click(object sender, EventArgs e)
        {
            ...
            {
                string[] allExistDevIDs = AppHubCenter.Instance.MDCellNameMgr.AllMotionDaqDevices();
