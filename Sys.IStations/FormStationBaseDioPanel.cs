    ...
    public partial class FormStationBaseDioPanel : Form
    {
        ...
        public void AdjustStationUI()
        {
            ...
            for (int i = 0; i < doNamesInView.Count; i++)
            {
                ...
                IDevCellInfo ci = AppHubCenter.Instance.MDCellNameMgr.GetDoCellInfo(doNamesInView[i]);
                do
                {
                    ...
                    if (dev.DioMCount <= ci.ModuleIndex)
                    {
                        lampDo.LampColor = LampButton.LColor.Gray;
                        lampDo.ForeColor = Color.Red;
                        lampDo.Enabled = false;
                        toolTips.SetToolTip(lampDo, "所属模块序号超限");
                        ShowTips(string.Format("DO: \"{0}\" 所属设备 \"{1}\" 所属模块Index = {2} 超出范围0~{3}", doNamesInView[i], ci.DeviceID, ci.ModuleIndex, dev.DioMCount == 0 ? 0 : dev.DioMCount - 1));
                        break;
                    }
                    IPlatModule_DIO md = dev.GetDio(ci.ModuleIndex);
            ...
            for (int i = 0; i < diNamesInView.Count; i++)
            {
                ...
                IDevCellInfo ci = AppHubCenter.Instance.MDCellNameMgr.GetDiCellInfo(diNamesInView[i]);
                do
                {
                    if (ci == null) //名称在配置中不存在
                    {
                    ...
                    if (dev.DioMCount <= ci.ModuleIndex)
                    {
                        ...
                        ShowTips(string.Format("DI: \"{0}\"  所属设备 \"{1}\" 所属模块Index = {2} 超出范围0~{3}", diNamesInView[i], ci.DeviceID, ci.ModuleIndex, dev.DioMCount == 0 ? 0 : dev.DioMCount - 1));
                        break;
                    }
                    IPlatModule_DIO md = dev.GetDio(ci.ModuleIndex);
        ...
        void UpdateStationUI()
        {
            ...
            for (int i = 0; i < doNamesInView.Count; i++)
            {
                ...
                IDevCellInfo ci = AppHubCenter.Instance.MDCellNameMgr.GetDoCellInfo(doName);
                do
                {
                    if (ci == null) //名称在配置中不存在
                    {
            ...
            for (int i = 0; i < diNamesInView.Count; i++)
            {
                ...
                IDevCellInfo ci = AppHubCenter.Instance.MDCellNameMgr.GetDiCellInfo(diName);
                do
                {
                    if (ci == null) //名称在配置中不存在
                    {
        ...
        void DOButtonClicked(object sender, EventArgs eventArgs)
        {
            ...
            if (dev.DioMCount <= ci.ModuleIndex)
            {
                ...
                ShowTips(string.Format("DO \"{0}\" 操作失败:所属设备 \"{1}\" 所属模块Index = {2} 超出范围0~{3}", doName, ci.DeviceID, ci.ModuleIndex, dev.DioMCount == 0 ? 0 : dev.DioMCount - 1));
                return;
            }
            IPlatModule_DIO md = dev.GetDio(ci.ModuleIndex);
            if (md.DOCount <= ci.ChannelIndex)
