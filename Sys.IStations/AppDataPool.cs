...
{
    public class AppDataPool : IDataPool
    {
        public AppDataPool()
        {
        ...
        ConcurrentDictionary<string, object> dictLists = new ConcurrentDictionary<string, object>();
        ...
        public bool RegistList(string key, Type itemType)
        {
            lock (dictListElementTypes)
            {
                ...
                return dictLists.TryAdd(key, item);
            }
        ...
        public object LockList(string key)
        {
            if (!dictListElementTypes.ContainsKey(key))
                return null;
